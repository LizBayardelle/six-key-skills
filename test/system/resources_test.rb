...
class ResourcesTest < ApplicationSystemTestCase
  ...
  test "creating a Resource" do
    ...
    check "Discipline" if @resource.discipline
    check "General" if @resource.general
    fill_in "Link url", with: @resource.link_url
    check "Mindset" if @resource.mindset
    check "Motivation" if @resource.motivation
    check "Planning" if @resource.planning
    fill_in "Resource type", with: @resource.resource_type
    check "Resourcing" if @resource.resourcing
    check "Time management" if @resource.time_management
